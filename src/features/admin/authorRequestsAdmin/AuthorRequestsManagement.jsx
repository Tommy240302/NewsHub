...
const AuthorRequestsManagement = () => {
  ...
  const fetchAuthorRequests = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:6969/admin/author-requests",
        {
  ...
  const handleAction = async (requestId, actionType) => {
    if (!requestId) {
      setSnackbar({
        open: true,
        message: "Lỗi: ID yêu cầu không hợp lệ.",
        severity: "error",
      });
      return;
    }
    ...
    try {
      ...
      setSnackbar({
        ...
        message:
          `Lỗi khi ${
            actionType === "approve" ? "phê duyệt" : "từ chối"
          } yêu cầu: ` + (err.response?.data?.errorMessage || err.message),
        severity: "error",
      });
    } finally {
            ...
            {requests.length === 0 ? (
              ...
              requests.map((request) => (
                ...
                >
                  ...
                  <TableCell>{request.paymentNumber}</TableCell>{" "}
                  {/* Thêm ô dữ liệu mới */}
                  <TableCell>
      ...
      >
        ...
        {selectedRequest && (
          <DialogContent dividers>
            ...
            <Typography variant="body1">
              <strong>Email người dùng:</strong> {selectedRequest.userEmail}
            </Typography>
            <Typography variant="body1">
              <strong>Số thanh toán:</strong>{" "}
              {selectedRequest.paymentNumber || "Không có"}
            </Typography>{" "}
            {/* Thêm dòng mới vào dialog */}
            <Typography variant="body1">
            ...
            <Typography variant="body1" mt={2}>
              <strong>Lý do:</strong> {selectedRequest.reason || "Không có"}
            </Typography>
            <Typography variant="body1">
              <strong>URL Hồ sơ:</strong>{" "}
              {selectedRequest.profileUrl ? (
                <Link
                  href={selectedRequest.profileUrl}
                  target="_blank"
            ...
            <Typography variant="body1">
              ...
              {selectedRequest.sampleArticles ? (
                <Link
                  href={selectedRequest.sampleArticles}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  {selectedRequest.sampleArticles}
